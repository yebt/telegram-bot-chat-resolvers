import {
  ...
  describeBot,
  type BotProfile,
} from "../../../core/telegram/bot-profile";
import {
  getMe,
  getUpdates,
  getWebhookInfo,
...
export function mountResolver(): void {
  ...
  function renderBot(bot: TelegramUser): void {
    ...
    identity.append(
      element("p", "text-3xl font-semibold leading-tight", profile.displayName),
    );
    identity.append(
      element("p", "entry__meta mt-1", profile.handle ?? "no public username"),
    );
    header.append(identity);

    const copyAllButton = element(
      "button",
      "stamp stamp--small shrink-0",
      "Copy all",
    );
    ...
    const facts = element(
      "div",
      "mt-2 grid grid-cols-1 gap-x-6 sm:grid-cols-2",
    );
    ...
    for (const capability of profile.capabilities) {
      capabilities.append(
        buildCapabilityChip(capability.label, capability.enabled),
      );
    }
  ...
  function renderTarget(): void {
    ...
    targetBody.append(
      buildCopyRow(
        "message_thread_id",
        topic?.threadId != null ? String(topic.threadId) : "—",
      ),
    );
  ...
  function buildChatEntry(chat: ResolvedChat): HTMLLIElement {
    ...
    button.type = "button";
    const selected =
      state.selection?.chat.id === chat.id && state.selection.topic === null;
    button.setAttribute("aria-pressed", String(selected));
    ...
    if (chat.isForum) {
      ...
      if (chat.topics.length > 0) {
        const topicList = element(
          "ul",
          "mt-2 flex flex-col gap-2 pl-3 border-l-2 border-[var(--edge)]",
        );
  ...
  async function resolve(): Promise<void> {
    ...
    if (!isLikelyBotToken(token)) {
      notify("error", {
        ...
        detail:
          "A bot token is a numeric id, a colon, and a long random string.",
    ...
    notify("info", {
      ...
      detail: "Asking Telegram about this bot…",
    });
