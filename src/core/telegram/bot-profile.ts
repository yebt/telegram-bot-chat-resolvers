...
export function describeBot(user: TelegramUser): BotProfile {
  ...
  return {
    ...
    capabilities: [
      ...
      {
        label: "Reads all group messages",
        enabled: user.can_read_all_group_messages ?? null,
      },
      {
        label: "Inline queries",
        enabled: user.supports_inline_queries ?? null,
      },
      {
        label: "Business connection",
        enabled: user.can_connect_to_business ?? null,
      },
