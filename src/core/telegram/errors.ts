...
export function fromApiResponse(payload: ApiResponse<unknown>, httpStatus: number): TelegramError {
  ...
  return new TelegramError(kind, description, { errorCode: code, description, retryAfter })
}
...
export function describeError(error: unknown): ErrorPresentation {
  ...
  switch (error.kind) {
    case 'aborted':
      return { title: 'Request cancelled', detail: 'The request was cancelled before it finished.' }

    case 'network':
      return {
        title: 'Could not reach Telegram',
        detail: 'The request to api.telegram.org failed before getting a response.',
    ...
    case 'webhook_conflict':
      return {
        ...
        detail: 'Telegram refuses getUpdates while a webhook is registered, so pending events cannot be read.',
